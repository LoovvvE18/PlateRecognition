...
def process_image_by_choice(img, choice):
    if choice == 1:
...
def main():
    # 在这里配置图片路径和处理方式数字
    img_path = "1.jpg"      # 你的图片路径
    choice = 8                 # 处理方式数字，1-灰度化 2-二值化 3-高斯模糊 4-Canny边缘检测 5-色彩反转

    img = img_read(img_path)
    if img is None:
        print("图片读取失败，请检查路径！")
        return
    result = process_image_by_choice(img, choice)
    winname = "result"
    # 若为灰度或单通道处理，直接展示，否则转为RGB显示防止色彩异常
